...
Shows sensitivity of noise performance to residual GLU-GABA correlation
...
def get_realistic_noise_values():
    ...
    try:
        ...
        print(f"Using realistic noise values: σ_GLU = {sigma_glu:.2e}, σ_GABA = {sigma_gaba:.2e}")
        return float(sigma_glu), float(sigma_gaba)
...
def main():
    ...
    sigma_glu, sigma_gaba = get_realistic_noise_values()
    
    # Compute sensitivity curve with realistic values
    rho, onsi = onsi_curve(sigma_glu, sigma_gaba, rho_min=-0.2, rho_max=0.2, n=201)
    ...
    # Formatting
    ax.set_xlabel(r'$\rho_{\mathrm{cc}}$ (GLU–GABA residual correlation)')
    ...
    ax.text(0.02, 0.02, f'σ_GLU = {sigma_glu:.2e}, σ_GABA = {sigma_gaba:.2e}', 
